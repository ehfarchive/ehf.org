...
| VisualDesigner | `anthropic/claude-opus-5` | reviewer and visual decision-maker: inspect the compact bundle, classify P0–P3 differences, and accept or block; a blocking P0/P1/P2 directs one BuildLead correction before the permitted recheck | mechanically operate the browser, produce capture bundles, edit application or CSS files, change route/content/test/billing/deployment contracts, or edit outside its review role |
...
**Owner-restored operational rule — 2026-08-10:** The owner confirmed Anthropic credits reset and restored VisualDesigner for pending Ticket 6 and Tickets 7–12 to the exact route `anthropic/claude-opus-5`. The prior 2026-08-09 temporary GPT-5.6 Sol exception through `cursor/gpt-5.6-sol-high` remains a historical fact, including its completed work; it is no longer an active route or substitute. All other unavailable-role cases, including the first provider error, stop the assigned visual work and require owner escalation; there is no automatic fallback, provider switch, or model substitution.

At most three agents run concurrently in total, including BuildLead. Work stays in the current OMP workspace routes only. Each handoff is accepted only when its route, manifest entry, and acceptance evidence agree.

## Visual-review efficiency protocol

VisualDesigner is the visual reviewer and decision-maker, not the mechanical browser operator. For each pending template review, BuildLead or ContentAssets mechanically produces one compact evidence bundle before the designer is invoked. The bundle contains matching source and local screenshots, measured geometry, masked pixel-difference metrics, console and network results, a contact sheet, and suspected P0–P3 differences. Mechanical production may be delegated between BuildLead and ContentAssets, but P0–P3 classification and acceptance/blocking remain VisualDesigner-exclusive; BuildLead performs any permitted correction.

Pending Ticket 6 and Tickets 7–12 use exactly one initial `anthropic/claude-opus-5` review for each remaining template ticket. Only when that initial review returns a blocking P0, P1, or P2 may BuildLead make exactly one correction, the evidence producer regenerate exactly one comparison, and Opus perform exactly one recheck. This is a hard cap of two Opus designer calls per remaining template ticket: record a warning after the first call, and escalate to the owner after the second call if it remains blocking. No P0, P1, or P2 defect may be accepted.

The failed GPT-5.6 Sol Ticket 6 resource-exhausted attempt remains recorded as a historical raw-terminal event, but it produced no judgment and neither replaces nor consumes Ticket 6's initial Opus review.

On the first provider error, inspect and preserve the raw terminal event, stop the assigned visual work, and escalate to the owner. Do not make a route-probe call, poll or wake a designer session, automatically retry or recover, repeat browsing or capture, or run extra aesthetic passes. VisualDesigner does not operate browser tooling mechanically. Run browser capture once per complete state matrix and image analysis once per bundle.
...
- **Visual-review efficiency:** VisualDesigner is reviewer-only and, after the owner's 2026-08-10 restoration, pending Tickets 6–12 use `anthropic/claude-opus-5`; prior GPT-5.6 Sol use through `cursor/gpt-5.6-sol-high` remains historical only. BuildLead/ContentAssets own one compact mechanical bundle. Each remaining template ticket gets one initial Opus review and, only for a blocking P0/P1/P2, one BuildLead correction and one Opus recheck; the cap is two calls with a warning after the first, then owner escalation. The Ticket 6 Sol resource-exhausted attempt remains a no-judgment record. Raw-terminal inspection and stop apply at the first provider error; route probes, polling, recovery, repeated browsing/capture, and extra aesthetic passes are forbidden. The independent blinded bakeoff is nonblocking and cannot alter the active route, acceptance criteria, or protocol without a later owner decision.
