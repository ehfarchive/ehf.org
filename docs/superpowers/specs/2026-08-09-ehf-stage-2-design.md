...
| VisualDesigner | GPT-5.6 Sol via `cursor/gpt-5.6-sol-high` (temporary owner-approved override) | reviewer and visual decision-maker: classify source/implementation differences and, under a bounded presentational sublease, prescribe or make one CSS correction | mechanically operate the browser, produce capture bundles, change route/content/test/billing/deployment contracts, or edit outside its sublease |
...
**Owner-approved operational override — 2026-08-09:** VisualDesigner temporarily runs GPT-5.6 Sol through the exact route `cursor/gpt-5.6-sol-high`. This named exception continues until the owner explicitly restores `anthropic/claude-opus-5` after Claude credits reset; that Opus route remains the intended future route and must not resume without that later owner decision. This is the only permitted temporary route exception. All other unavailable-role cases stop their assigned work; there is no automatic fallback, provider switch, or model substitution.
...
- **Visual-review efficiency:** VisualDesigner is reviewer-only; it temporarily uses GPT-5.6 Sol through `cursor/gpt-5.6-sol-high` until the owner explicitly restores the intended future Opus route after Claude credits reset. The independent blinded bakeoff is nonblocking and cannot alter the active route, acceptance criteria, or protocol without a later owner decision. BuildLead/ContentAssets own compact mechanical evidence bundles. The protocol has one correction/recheck, fresh context, batched tools, warnings at 6/10, caps at 8/12, raw-event inspection before retry, immediate first-429 stop, structured cap blocker, complete ledger fields, and no quality concession for a call/cost target.
